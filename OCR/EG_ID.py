...
def predict_id(path):
    ...
    results = model(path, save=True, conf=0.6, imgsz=640, show=False, save_crop=True, project=RUNS_DIR, name='detect')
    return results
...
def save_top_right_boxes(path, save_dir, results):
    ...
    filtered_boxes = [box for box in boxes_info 
    ...
    for i, box_info in enumerate(top_3_boxes, start=1):
        ...
        print(f"Saved top box #{i} ({box_info['class_name']}) to: {output_path}")
...
def main():
    ...
    if path:
        ...
        firstname_img_path = os.path.join(save_dir, 'crops', '1', os.path.basename(path))
        secondname_img_path = os.path.join(save_dir, 'crops', '2', os.path.basename(path))
        location_img_path = os.path.join(save_dir, 'crops', '3', os.path.basename(path))
        ...
        numbers, detections = extract_digits_from_id(os.path.join(save_dir, 'crops', 'egyptian-id', os.path.basename(path)), conf_threshold=0.25)
