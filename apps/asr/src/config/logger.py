...
def setup_logging():
    ...
    # If relative path, resolve it relative to asr root; otherwise use as absolute
    if not os.path.isabs(log_dir):
        # Resolve log path relative to the asr/ root directory (3 levels up from this file)
        asr_root = Path(__file__).resolve().parent.parent.parent
        log_path = (asr_root / log_dir).resolve()
    else:
